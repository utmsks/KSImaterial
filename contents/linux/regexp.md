...
実習用Linuxシステムの各自のホームディレクトリの下に適当な名前の作業用ディレクトリを作り、そこに次のリンク先（204号室実習用ネットワーク内からのみ）にある説明ファイル(`3201.txt`)アーカイブファイル(`files.zip`)をダウンロード・展開してください。
...
What is that?
```
（`wrong.txt`というファイルは用意してありませんが、自分で示されている内容のファイルを作ってみてください。以下同様）
