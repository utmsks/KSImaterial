...
##### `apt`のプロキシ設定
<a name="apt_proxy"></a>次に、アップデートマネージャーコマンド`apt`用のプロキシ設定を行います。**Ctrl + Alt + T**の3つのキーを同時押しします。このショートカットはターミナルと呼ばれるプログラムを呼び出します。
...
##### `apt`を使ったアップデート
`apt`を用いたアップデートをします。204のインターネットは特殊なため、まず次のコマンドを打ちます。
