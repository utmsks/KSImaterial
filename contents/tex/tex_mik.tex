...
例えば，\texttt{U+E01$xy$}を<\texttt{xy}>と表記した時\footnote{%
  ...
  という表記ができるので，%
  \TeX{}ソース中に「\texttt{東京都葛󠄁\textasciicircum\textasciicircum\textasciicircum\textasciicircum\textasciicircum 0e0101飾区}」と書くこともできる．
}，
\begin{lstlisting}
...
実習資料では「数式中には和文文字は書けない」と書いた．
一方，Unicodeにはたくさんの数学記号が収録されている．
せっかくUnicodeに数学記号があるのだから，\TeX{}ソース中にそれらを直接記述できると便利である．
\LuaTeX{}と\XeTeX{}では，\PKG{unicode-math}パッケージを使うとUnicodeの数学記号を直接記述することができるようになる．
また，PDF中の数式からコピー＆ペーストした時にUnicodeの数学記号が含まれる文字列がペーストされるようになる．

TODO: 書きたい

\newpage
\subsection{\TikZ{}パッケージ}
