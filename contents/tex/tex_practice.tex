...
\FNAME{updmap-sys}のように\FNAME{-sys}をつけたコマンドを使う．
...
% 	    （画像）}」に一覧が載っている．
 ...
 \item[半角カナ]
「ｱｲｳ」のようないわゆる半角カナは，しばしば「電子メールでは使うべきではない」と言われている．
...
{\Large 廹既海溫㑨愙𦥑𡈽}などのJIS第3・第4水準漢字が追加されている．
...
\end{document}
\end{lstlisting}
を\emph{UTF-8}で記述（「Ⅰ」は全角ローマ数字），\pLaTeX でタイプセットし，PDFファイルを作ると
\begin{quote}
 \large 「âĚă」，「å¡Ě」，「ï¡śï¡šï¡ş」
\end{quote}
のような出力が得られる．
Windowsの機種依存文字や半角カナが正しく和文文字と認識されていないことがわかるだろう．
...
また，半角カナ「ｱ」は\texttt{EF BD B1}と符号化されるが，
...
使い方は，あまりおすすめしない．}．
 \item[半角カナ] タイプセットで，つまりdviファイルの段階で例えば「ｱｲｳ」が「śšş」と化ける．
\begin{curve}
 これも，Shift\_JISでは半角カナ「ｱｲｳ」は\texttt{B1 B2 B3}と符号化されるが，
...
八登氏による「\href{https://qiita.com/zr_tex8r/items/5c14042078b20edbfb07}{[改訂新版]u\pLaTeX を使おう}」
を参照．
...
\item \emph{\TeX ソースの文字コードを\emph{UTF-8}とすること．}\\
  macOSやLinuxでは標準でこの設定になっていることが多いが，
...
% \hfill または，
 \texttt{texdoc 01uptex}で参照できる\FNAME{01uptex_doc_utf8.txt}
\end{quote}
...
TODO: \texttt{tex\_mik.tex} の内容を移植したい
...
\href{https://ctan.org/pkg/xypic}{\emph{\Xy-pic}}%
というパッケージを使うのが便利である．
...
さらに阿部紀行氏のページ%
「\href{https://www.ms.u-tokyo.ac.jp/~abenori/}{あべのりページ}」%
...
\begin{itemize}
 ...
 \verb+^+で，右側には\verb+_+で通常の添字のように記述する．
 上のソース中の\verb+\ar[l]^{\pi_1}+と\verb+\ar[r]_{\pi_2}+を比較して欲しい．
 \item \verb+@{...}+は線種の指定である．通常の矢の他に可換図式で良く使うものとしては
\[
...
\item 矢印を少し曲げたい場合には，\verb+@/^/+,~\verb+@/_/+でそれぞれ
進行方向左側・右側に曲がる．
\end{itemize}
