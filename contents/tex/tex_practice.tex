...
\item 伝統的な\pLaTeX は知っているが，\pLaTeX が利用できない場合への対処法や，
 最近の\TeX の発展について知りたい人は\prettyref{sec:other-engines}か，
...
正：$a+b+\cdots+z$
\end{LTXexample}
なお，Unicodeにある数学記号をそのまま\TeX{}ソース中で使いたい，という場合は，実習資料の「\pTeX{}系列以外による日本語文書作成」(\FNAME{tex_mik.pdf})にある\PKG{unicode-math}パッケージの紹介を読むと良いかもしれない．
...
このあたりは\url{https://togetter.com/li/1124861}などを参考にして，各自調べること\footnote{%
  結論を言うと，文書クラスがBXjsclsであればクラスオプション\texttt{label-section=modern}を指定し，\cs{thesection}の代わりに\cs{pre}/\cs{postsectionname}をカスタマイズすれば良い．
...
\newpage
\section{\pTeX 系列以外の\TeX エンジンについて}
...
そこで，\TeX{}のマクロによって命令や文法を追加し，文書を書きやすくする\emph{マクロパッケージ}が多数開発されている．
\LaTeX{}はそうしたマクロパッケージの一種である．
例えば，文書クラス\cs{documentclass}や環境（\cs{begin}, \cs{end}コマンド）等の概念は，\LaTeX{}によって提供されたものである．
\LaTeX{}以外の枠組みとしては，plain~\TeX{}や\ConTeXt{}がある．

\begin{center}
  \begin{tikzpicture}
    \node[left] at (0,.5) {\TeX{}エンジン};
    \draw (0,0) rectangle node {\TeX82, ($\varepsilon$-)(u)\pTeX, \pdfTeX, \XeTeX, \LuaTeX{}等} (9,1);
    \node[left] at (0,2) {マクロパッケージ};
...
\LuaTeX{}の場合は\LuaTeX-jaが，\XeTeX{}の場合は\PKG{xeCJK}や\PKG{ZXjatype}がそれである．

ちなみに，この資料（\FNAME{tex_practice.pdf}）は\LuaTeX{} (\& \LuaTeX-ja) を用いて組版されている．

\subsection{文書クラス}

\PKG{jsarticle}をはじめとする「\pLaTeXe 新ドキュメントクラス」(\PKG{jsclasses}) はその名の通り\pLaTeX{}系専用である．
よって，\LuaTeX{}や\XeTeX{}等のエンジンを用いて日本語文書を書く際には，\PKG{jsclasses}以外の文書クラスを使うことになる．
というわけで，日本語文書を書くのに使える文書クラスを（\pLaTeX{}専用のものも含めて）いくつか挙げておく．
\begin{description}
\item[jclasses]
  アスキーによる，\pTeX{}用の文書クラス．(u)\pLaTeX{}用
\item[jsclasses]
  奥村晴彦氏による「\pLaTeXe 新ドキュメントクラス」．(u)\pLaTeX{}用．
\item[ltjsclasses]
  \PKG{jsclasses}を\LuaTeX-ja用に移植したもの．
\item[BXjscls]
  \PKG{jsclasses}を拡張したもの．
  (u)\pLaTeX, \pdfLaTeX, \XeLaTeX, \LuaLaTeX{}と，幅広いエンジンに対応している．
\item[jlreq]
  「日本語組版の要件」\footnote{%
    \url{https://www.w3.org/TR/jlreq/ja/}
  }を満たすことを目指す文書クラス．\LuaTeX-ja, \pLaTeX, \upLaTeX{}対応．
\end{description}

その他，特定の用途に使われる文書クラスもある．
\begin{description}
\item[beamer]
  \LaTeX{}でスライドを作るための文書クラス．
\item[standalone]
  文書ではなく単体の図を出力できる文書クラス．
  数式や（\TikZ{}で描いた）図を他の文書に貼り付けたり，画像化して他のアプリケーションに貼り付けたり，という目的で使える．
\end{description}

% DVIwareの指定についてもどこかに書く？
...
自動判別が不安な場合は，明示的に\texttt{-kanji=utf8}を指定する（\TeX ソースがUTF-8の場合）か，\TeX{}ソースをBOM付き\footnote{%
  一般論としてUTF-8の「BOM」は8ビットクリーンなソフトウェアにおいて問題を引き起こす可能性があるので要注意だが，\TeX~Live~2018以降の\TeX{}の場合はUTF-8のBOMは正常に解釈される（一方，古い\TeX~Liveでは\pdfTeX{}がUTF-8のBOMを認識しない）．
  よって，最新の\TeX~Liveで処理させる\TeX{}ファイルに関してはUTF-8のBOMはあってもなくても良い．
}UTF-8で保存する．
\begin{lstlisting}
...
これ以上詳しくは「\pTeX 系列以外による日本語文書作成」(\FNAME{tex_mik.pdf}) を参照されたい．
...
% > extractbb fig1.png
% > extractbb fig2.pdf
% という風に画像ファイルごとにextractbbを実行してBoundingBoxの情報（.xbbファイル）を生成する
% 必要があったが，TeX Live 2015以降ではデフォルトで extractbb は自動実行される．
% 昔のTeX Liveでも -shell-escape を指定すれば extractbb が自動実行されたが，
% -shell-escape は文書処理中に任意の外部コマンド実行を許可するオプションなので，盲目的に使うには危険である．
% 参考：https://texwiki.texjp.org/?extractbb%20%E3%81%AE%E8%87%AA%E5%8B%95%E5%AE%9F%E8%A1%8C%E8%A8%B1%E5%8F%AF%E3%81%AE%E8%A8%AD%E5%AE%9A
...
% \subsection{\TeX 自体を深く知りたい方へ}

% \begin{itemize}
% \item The \TeX{}book
% \item \TeX{} by Topic
% \item \LaTeX の実装：source2e
% \end{itemize}
% TA小話
% TeX WikiのTeX入門

\clearpage
\subsection{\pTeX{}系列で縦書き等をする場合に発生する問題とその対処}
% plautopatchの紹介

欧米で開発されているパッケージは日本語組版の事情，特に縦書きを考慮していない場合が多い．
例えば，\PKG{array}パッケージと，\pLaTeX{}の\PKG{plext}パッケージは，どちらも\LaTeX{}標準の\ENV{tabular}環境を拡張しており，お互いに相容れない．
他にも\pLaTeX{}で問題が起こりうるパッケージが多数あり，それらに対して対策をするパッケージが開発されている．
例えば，\PKG{array}パッケージ+\PKG{plext}パッケージの問題に対しては\PKG{plextarray}パッケージ，という具合である．

ただ，問題が起こりうるパッケージに対していちいち対策パッケージを調べて読み込むのは面倒である．
この手間を省くために山下弘展氏が製作しているのが\href{https://www.ctan.org/pkg/plautopatch}{\PKG{plautopatch}パッケージ}である．
% https://github.com/aminophen/plautopatch
% \PKG{plautopatch}を使うには，\cs{documentclass}の前に\verb+\RequirePackage{plautopatch}+を書く．
\begin{itemize}
\item 山下弘展「日本語の\LaTeX で幸せになる，かもしれない方法」, TeXConf 2018
  \url{https://aminophen.github.io/slide/hytexconf18.pdf}
\end{itemize}
...
% 以下はボツ：
% 高機能作図パッケージ\TikZ{}の機能に，「異なる\TikZ{}図形の位置を参照する」というものがある．
% \begin{LTXexample}[pos=b]
% \[a^{\tikz[remember picture] \node[draw,inner sep=1pt] (sisuu) {$\scriptstyle b$};}\]
% べき乗の\tikz[remember picture,baseline=(kono.base)] \node[draw,inner sep=1pt] (kono) {この};部分を指数と呼ぶ．
% \tikz[remember picture,overlay] \draw[->] (kono) -- (sisuu);
% \end{LTXexample}
% \begin{itemize}
% \item \href{https://zrbabbler.hatenablog.com/entry/20130222/1361495591}{TikZ は dvipdfmx をどこまでサポートするか？ (1)}
% \item \href{https://zrbabbler.hatenablog.com/entry/20130302/1362228901}{TikZ は dvipdfmx をどこまでサポートするか？ (2)}
% \end{itemize}
% そこで\PKG{pxpgfmark}を使う必要がある……と言いたいところだったが，
% TeX Live 2018では pgfsys-dvipdfmx.pdf が改修されたのか，pxpgfmarkがなくてもremember picture系の機能が使えるようだ．
% https://github.com/pgf-tikz/pgf/commit/bf7d99e9b81034a3dfbaa880f19a5f1cd132a546#diff-b929ac9caab8c299d30b6967d4a65e30
% このコミットで \pdftexversion に対する検査が \pdfsavepos に対する検査に変わったためか．
